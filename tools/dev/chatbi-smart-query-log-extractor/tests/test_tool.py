...
class ExtractorTests(unittest.TestCase):
    ...
    def test_render_html_shows_navigation_prompt_and_missing_markers(self) -> None:
        ...
        self.assertIn("const FLOW_SWITCH_OVERSCROLL_THRESHOLD = 140;", html)
        self.assertIn("const FLOW_SWITCH_COOLDOWN_MS = 360;", html)
        self.assertIn("function resetFlowBoundaryOverscroll()", html)
        self.assertIn("if (now - lastFlowSwitchAt < FLOW_SWITCH_COOLDOWN_MS) {", html)
        self.assertIn("flowBoundaryOverscroll += Math.abs(event.deltaY);", html)
        self.assertIn("if (flowBoundaryOverscroll < FLOW_SWITCH_OVERSCROLL_THRESHOLD) {", html)
