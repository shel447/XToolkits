...
- `*.html`: 先按问题分组、再按调用 ID 分块的排障页面；`IR 表定义`、`最终 Prompt`、`生成 IR 结果` 默认折叠，按需展开查看
